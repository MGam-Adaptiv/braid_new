...
export const saveActivity = async (userId: string, activity: Partial<Activity>): Promise<string> => {
  try {
    ...
    const activityData = {
      ...
      activityTypeName: (activity as any).activityTypeName || null,
      activityCategory: (activity as any).activityCategory || activity.category || null,
      activityFormat: (activity as any).activityFormat || null,
...
export const saveMaterial = async (teacherId: string, materialData: { title: string; publisher: string; bookTitle: string; unitTags: string[]; labelTags: string[]; vocabulary: string[]; coreVocabulary?: string[]; secondaryVocabulary?: string[]; grammar: string[]; topic: string; level: string; pageCount: number; ocrTexts?: string[]; pageNumbers?: (number | null)[]; }): Promise<string> => {
  try {
    const payload = {
      ...
      pageNumbers: materialData.pageNumbers || [],
      isFavorite: false,
      createdAt: serverTimestamp(),
      lastUsedAt: null,
      timesUsed: 0
