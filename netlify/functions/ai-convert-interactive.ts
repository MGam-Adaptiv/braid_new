...
- ABSOLUTE RULE — ONE BLANK PER QUESTION: Each fill-blank question object must contain EXACTLY ONE ___ blank (three underscores — never __ or ____). Count the blanks in EVERY sentence before finalising — do not skip this check. If a sentence has two gaps, you MUST split it into two separate question objects — one per blank, each with its own correctAnswer. A sentence with two ___ is a critical error that breaks the student interface. Never put two ___ in a single question string under any circumstances.
